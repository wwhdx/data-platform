...
> **版本**：v1.1 · **状态**：**L1 已实施**（Phase A）；L2–L6 待实施  
> **最后更新**：2026-05-19  
> **关联**：[功能实现与设计总览.md](../功能实现与设计总览.md) §7、[storage-strategy.md](../storage-strategy.md) §4.2、[原始数据本地导出与镜像方案.md](./原始数据本地导出与镜像方案.md)、`src/processors/dedup.ts`、`src/scheduler/progress.ts`
...
5. **溯源字段未写**：`Scheduler` 未给 `RawDocument.collectionJobId` 赋值，按 job 追溯弱。
...
| B5 | `pnpm cli collect` | 写入 `collectionJobId` 至新插入行 |

**验收**：`pnpm cli jobs --limit 1` 可见 fetched/inserted/skipped；`psql` 可查 JSONB。

### Phase C — 文件日志 + 事件表（P2，约 1–1.5d）

| 项 | 说明 |
|----|------|
| C1 | `src/collect/logWriter.ts`（新）NDJSON 落盘 |
| C2 | `collection_job_events` + `cli jobs --job-id --events` |
| C3 | `--verbose` 抽样 `skip_sample` |

### Phase D — 统一 Logger（P2，可与 C 并行）

| 项 | 说明 |
|----|------|
| D1 | `src/lib/logger.ts`（pino）；`LOG_LEVEL` env |
| D2 | `dedup` / `embed` / `mirror` 改用 `logger.warn` |
| D3 | `.env.example`、`CLAUDE.md` 环境变量表 |
...
同步落点：`.env.example`、`CLAUDE.md` 环境变量表、[功能实现与设计总览.md](../功能实现与设计总览.md) §10（实施后）。
...
单测：`dedup` 返回 skippedCount → scheduler 累加；smoke：`POST /collect?stream=true` 解析新字段。
...
|------|------|
| 本文 | 文首 **状态** → ✅；§5 勾选 |
| [功能实现与设计总览.md](../功能实现与设计总览.md) | §7.3、§14、§15 |
| [实施进度总览.md](./实施进度总览.md) | §3 L 轨任务状态 |
