...
> **版本**：v1.6 · **最后同步**：2026-05-19  
> **代码基线**：`pnpm test:run` 121+ passed；迁移 `001`–`009`；YAML **12** 源 / 运行时 **4** Connector  
...
| `010_fetch_provenance.sql`（□ D5） | `raw_documents.fetch_provenance` JSONB；HTTP 溯源入库 |
...
| RawDocument | 不可变 JSONB + 可选 HTTP 溯源 | ✅ `raw_documents`；🟡 `fetch_provenance` 见 [D5](./plans/原始数据本地导出与镜像方案.md#47-数据来源溯源provenanced5) |
...
| 导出/入库数据来源溯源（D5） | □ 设计定稿 | `fetch_provenance` + 信封 `schemaVersion: 2`；[§4.7](./plans/原始数据本地导出与镜像方案.md#47-数据来源溯源provenanced5) |
