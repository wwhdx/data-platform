...
> **版本**：v1.4 · **最后同步**：2026-05-19  
> **代码基线**：`pnpm test:run` 98 passed；迁移 `001`–`007`；YAML **12** 源 / 运行时 **4** Connector  
...
| RawDocument | 不可变 JSONB | ✅ `raw_documents` |
...
| **采集日志与可观测性（L 轨）** | 🟡 L1 已落地 | [方案](./plans/采集日志与可观测性设计方案.md)；CLI 展示 `skippedDuplicate`；L2 stats 待做 |

**注意**：Embedding 为 fire-and-forget；无队列、无自动重试（见 [下一阶段实施方案](./plans/下一阶段实施方案.md) §5）。本地磁盘为 PG 副本，检索仍以 `raw_documents` 为准。

### 7.3 采集日志与可观测性（L1 已落地 · L2+ 待实施）

> 详案：[plans/采集日志与可观测性设计方案.md](./plans/采集日志与可观测性设计方案.md)

**L1（2026-05-19）**：`progress` / `source_done.stats` 含 `fetched`、`inserted`、`skippedDuplicate`；CLI 显示「重复跳过」及全重复时的说明行。代码：`src/scheduler/progress.ts`、`src/scheduler/index.ts`、`src/cli/index.ts`。

**仍待做**：`collection_jobs.stats` 持久化（L2）、落盘日志（L4）、`collection_job_id` 写入（L3）；Embedding / D2 失败仍为 `console.error`。
...
| 层级 | 命令 | 依赖 |
|------|------|------|
| L0 | `pnpm test:run` | 无 DB（含 smoke inject） |
| L1 | `pnpm cli config validate` | YAML |
| L2 | `pnpm smoke:live` | Docker DB + serve |
...
- 契约测试：`src/__tests__/smoke/engineCore-adapter.test.ts`
...
| 采集可观测性不足 | 「抓取 N、入库 0」难区分重复跳过 vs 真故障；见 [L 轨方案](./plans/采集日志与可观测性设计方案.md) |
