...
export interface OecdCatalogRow extends Record<string, unknown> {
  agency: string;
  flow_id: string;
  name: string | null;
...
export interface OecdCatalogUpsertInput {
  agency: string;
  flowId: string;
  name?: string | null;
  description?: string | null;
...
export async function upsertOecdCatalogDataflow(
  input: OecdCatalogUpsertInput,
): Promise<void> {
  await query(
    `INSERT INTO oecd_catalog_dataflows (
...
export async function listOecdCatalogDataflows(opts?: {
  agencyPrefix?: string;
  collectEnabledOnly?: boolean;
}): Promise<OecdCatalogRow[]> {
...
export async function searchOecdCatalogByName(
  q: string,
  limit: number,
): Promise<OecdCatalogRow[]> {
  const pattern = `%${q}%`;
...
export async function countOecdCatalogByAgency(): Promise<
...
export async function countOecdCatalogDataflows(): Promise<number> {
  const res = await query<{ count: string }>(
    `SELECT COUNT(*)::text AS count FROM oecd_catalog_dataflows`,
  );
