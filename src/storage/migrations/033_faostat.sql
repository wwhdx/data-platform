...
CREATE TABLE IF NOT EXISTS faostat_catalog_dataflows (
  agency                TEXT NOT NULL DEFAULT 'FAO',
...
CREATE INDEX IF NOT EXISTS idx_faostat_catalog_tier ON faostat_catalog_dataflows(tier) WHERE collect_enabled;
...
FROM raw_documents
WHERE source_id IN (
  'fred', 'worldbank', 'eia', 'eurostat', 'oecd', 'imf', 'ecb',
  'census', 'bea', 'faostat'
);
