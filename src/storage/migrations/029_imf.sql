...
CREATE TABLE IF NOT EXISTS imf_catalog_dataflows (
...
CREATE INDEX IF NOT EXISTS idx_imf_catalog_agency ON imf_catalog_dataflows(agency);
CREATE INDEX IF NOT EXISTS idx_imf_catalog_tier ON imf_catalog_dataflows(tier) WHERE collect_enabled;
...
SELECT
  ...
  collection_job_id
FROM raw_documents
WHERE source_id IN ('fred', 'worldbank', 'eia', 'eurostat', 'oecd', 'imf');
